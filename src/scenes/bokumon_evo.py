...
class BokuEvo:
        ...
        if not self.cancel:
            if self.show_msg and self.msg_timer.run:
                ...
                if self.pressed_x:
                    self.cancel = True
                    if (
                        self.atual_bokumon.evolved[0][0]
                        and not self.atual_bokumon.evolved[0][1]
                    ):
    ...
    def activate(self, first_time=False):
        ...
        self.msg_timer.active()
        for i in range(300):
            self.list_gas.append(
                [
                    screen_width / 2 + randint(-100, 100),
                    screen_height / 2 + randint(-100, 100),
                ]
            )
    ...
    def up_info(self):
        if self.up_bokumon:
            if self.msg_timer.run:
                ups = [
                    "Life ",
                    f"+{self.player.atual_bokumon.atrib_ups[0]}",
                    "Attack ",
                    f"+{self.player.atual_bokumon.atrib_ups[1]}",
                    "Defense ",
                    f"+{self.player.atual_bokumon.atrib_ups[2]}",
                    "Speed ",
                    f"+{self.player.atual_bokumon.atrib_ups[3]}",
                    "Crit.Chance ",
                    f"+{self.player.atual_bokumon.atrib_ups[4]}",
                ]
            else:
                ups = [
                    "Life ",
                    f"{self.player.atual_bokumon.life}",
                    "Attack ",
                    f"{self.player.atual_bokumon.attack}",
                    "Defense ",
                    f"{self.player.atual_bokumon.defense}",
                    "Speed ",
                    f"{self.player.atual_bokumon.speed}",
                    "Crit.Chance ",
