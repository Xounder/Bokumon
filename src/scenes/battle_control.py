...
class BattleControl:
    ...
    def catch_bokumon_chance(self, ball_effective):
        ...
        self.ball_shake_times = 0
        max_catch = (
            (3 * self.wild_bokumon.life - 2 * self.wild_bokumon.life)
            * (self.wild_bokumon.catch_rate * ball_effective)
        ) / (3 * self.wild_bokumon.life)
        max_catch = (2**20 - 2**4) / (math.sqrt(math.sqrt((2**24 - 2**16) / max_catch)))
        catch_value = (
            (3 * self.wild_bokumon.life - 2 * self.wild_bokumon.atual_life)
            * (self.wild_bokumon.catch_rate * ball_effective)
        ) / (3 * self.wild_bokumon.life)
        catch = (2**20 - 2**4) / (math.sqrt(math.sqrt((2**24 - 2**16) / catch_value)))
        for i in range(3):
            rand = randint(
                0, round(max_catch * (2 - (self.wild_bokumon.catch_rate / 1000) * 3))
            )
    ...
    def critical_hit(self, damage, attack_bokumon):
        ...
        if (
            chance <= attack_bokumon.critical_chance
            and attack_bokumon.critical_chance > 0
        ):
    ...
    def player_attack_turn(self, select_move):
        self.wild_damage = self.attack_bokumon(
            self.player_bokumon, self.wild_bokumon, select_move
        )
    ...
    def attack_bokumon(self, attack_bokumon, defend_bokumon, select_move=""):
        ...
        damage = (
            (self.critical_hit(2 * attack_bokumon.level, attack_bokumon) / 5 + 2)
            * select_move[1]
            * atk
            / defense
        )
    ...
    def get_exp_points(self):
        exp = (
            self.wild_bokumon.level * randint(1, 3)
